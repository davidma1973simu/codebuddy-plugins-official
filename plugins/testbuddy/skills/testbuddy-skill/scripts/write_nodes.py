...
if __name__ == "__main__":
    ...
    if action not in ["add", "update", "delete"]:
        print(
            json.dumps(
                {"status": "error", "msg": "action 必须是 add/update/delete 之一，当前值：{}".format(action)},
                ensure_ascii=False,
                indent=2,
            )
        )
    ...
    try:
        ...
        if not input_data or not input_data.strip():
            print(json.dumps({"status": "error", "msg": "未接收到输入数据"}, ensure_ascii=False, indent=2))
            sys.exit(1)
        ...
        try:
            ...
            print(
                json.dumps(
                    {"status": "error", "msg": "JSON解析失败: {}".format(error_msg)}, ensure_ascii=False, indent=2
                )
            )
            sys.exit(1)
